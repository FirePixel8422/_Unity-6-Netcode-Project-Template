...
namespace Fire_Pixel.Networking
{
    ...
    public struct PlayerIdDataArray : INetworkSerializable
    {
        ...
        public readonly int GetPlayerGameId(ulong networkId)
        {
...
#if UNITY_EDITOR
            string errorString = "Cant Convert Id: " + networkId + ", networkIds are: ";
            for (int i = 0; i < playerCount; i++)
            {
        ...
        public readonly ulong GetPlayerNetworkId(int gameId)
        {
            return networkIds[gameId];
        }

        public readonly string GetUserName(int gameId)
        {
            return usernamesOnServer[gameId].ToString();
        }

        public readonly string GetPlayerGUID(int gameId)
        {
            return guidsOnServer[gameId].ToString();
        }
        ...
        private readonly void SerializeStringArray<T>(BufferSerializer<T> serializer, ref string[] array) where T : IReaderWriter
        {
